...
This document is the production-boundary record for the headless remote-access
platform. It does not describe or ship a phone UI. The public gateway remains
loopback-only; LAN, direct, and relay adapters terminate mutually authenticated
encryption before reaching a fixed supervised gateway destination.
...
- `latch remote-access relay disable` independently prevents new relay
  admission while leaving LAN/direct access available. Control-plane adapters
  must check `relay_enabled` before ticket issuance.
- LAN handshakes have a ten-second deadline and a 32-connection cap. The relay
  separately caps tickets, endpoints, frame size, rate-window frames/bytes,
  and queued ciphertext. Duplicate endpoint admission and use after ticket
  expiry are rejected; disconnect releases capacity.
...
| --- | --- | --- |
| Pairing expiry/replay/cancellation and secret-at-rest | Rust remote-access unit tests | Pass |
...
| Direct setup, reconnect, path migration | deterministic two-endpoint UDP/state tests | Pass |
| Forced relay, E2E secrecy, quotas, expiry, disconnect | Rust relay cryptographic/authorization tests | Pass |
...
| Same-LAN path on the CI host | headless listener/direct probe smoke | Pass |
...
| Symmetric NAT / UDP blocked | Forced-relay state and crypto path, not a physical carrier network | Pass at protocol level |
...
The code is ready for a separate basic-phone-chat implementation against the
stable transport boundary. Production rollout remains gated on an independent
security assessment, Apple signing/notarization in the release pipeline, a
deployed control plane and opaque relay, and measured physical-network/load/
soak results. Remote access and relay must remain off by rollout policy until
those external gates are satisfied. These are deployment evidence gaps, not
permissions for the future phone client to bypass the documented boundaries.
