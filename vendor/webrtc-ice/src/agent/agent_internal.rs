...
// Latch patch: a TURN permission transaction can wait longer than the ICE
// deadline. Keep bounded STUN queues per local socket so it cannot block
// checks, nomination or responses on the other candidates.
struct StunWrite {
    bytes: Vec<u8>,
...
struct StunWriter {
    sender: mpsc::Sender<StunWrite>,
    task: tokio::task::JoinHandle<()>,
}
impl Drop for StunWriter {
    fn drop(&mut self) {
        self.task.abort();
    }
}
...
pub struct AgentInternal {
    stun_writers: Mutex<HashMap<String, StunWriter>>,
...
impl AgentInternal {
    pub(super) fn new(config: &AgentConfig) -> (Self, ChanReceivers) {
        ...
        let ai = AgentInternal {
            stun_writers: Mutex::new(HashMap::new()),
    ...
    pub(crate) async fn delete_all_candidates(&self) {
        // Abort blocked writes before close needs the TURN allocation mutex.
        self.stun_writers.lock().await.clear();
        {
            let mut local_candidates = self.local_candidates.lock().await;
    ...
    pub(crate) async fn send_stun(
        ...
        let mut writers = self.stun_writers.lock().await;
        let writer = match writers.entry(local.id()) {
            std::collections::hash_map::Entry::Occupied(entry) => entry.into_mut(),
            std::collections::hash_map::Entry::Vacant(entry) => {
                let (sender, mut receiver) = mpsc::channel::<StunWrite>(32);
                let local = Arc::clone(local);
                let mut closed = local
                    .get_closed_ch()
                    .lock()
                    .await
                    .as_ref()
                ...
                let task = tokio::spawn(async move {
                    let writes = async {
                        while let Some(write) = receiver.recv().await {
                            // ICE retries checks; sending an old queued packet
                            // after a slow permission completes is unnecessary.
                            if write.queued_at.elapsed() > Duration::from_secs(1) {
                                continue;
                            }
                            if let Err(err) = local.write_to(&write.bytes, &*write.remote).await {
                                log::trace!("failed to send queued STUN message: {err}");
                            }
                        }
                    };
                    tokio::select! {
                ...
                entry.insert(StunWriter { sender, task })
            }
        };
        // Never await queue capacity on the check loop or an inbound reader.
        if writer
            .sender
            .try_send(StunWrite {
                bytes: msg.raw.clone(),
            ...
            .is_err()
        {
            log::trace!("STUN write queue full or closed; ICE will retry");
        }
    }
