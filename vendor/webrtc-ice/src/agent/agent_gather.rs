...
impl Agent {
        ...
        for t in &params.candidate_types {
            match t {
                ...
                CandidateType::Relay => {
                    ...
                    let network_types = params.network_types.clone();
                    ...
                    tokio::spawn(async move {
                        ...
                        Self::gather_candidates_relay(urls, network_types, net, agent_internal)
                            .await;
                    });
                }
                _ => {}
    ...
    async fn gather_candidates_srflx(params: GatherCandidatesSrflxParams) {
        ...
        for network_type in network_types {
            ...
            for url in &urls {
                // Only UDP STUN/TURN endpoints can answer plaintext UDP
                // binding requests. TCP and TLS ports otherwise cost a full
                // gathering timeout on every application request.
                if url.proto != ProtoType::Udp
                    || (url.scheme != SchemeType::Stun && url.scheme != SchemeType::Turn)
                {
                    continue;
                }
                let network = network_type.to_string();
    ...
    pub(crate) async fn gather_candidates_relay(
        ...
        for url in urls {
            ...
            // The client-to-TURN socket family is independent of the relay
            // address family. IPv6-only cellular paths can obtain IPv4 relays.
            for network_type in network_types.iter().copied().filter(|n| n.is_udp()) {
                let url = url.clone();
                let net2 = Arc::clone(&net);
                let agent_internal2 = Arc::clone(&agent_internal);
                ...
                tokio::spawn(async move {
                    ...
                    let turn_server_addr = if url.host.contains(':') {
                        format!("[{}]:{}", url.host.trim_matches(['[', ']']), url.port)
                    } else {
                    ...
                    {
                        let bind_address = if network_type.is_ipv4() {
                            "0.0.0.0:0"
                        } else {
                            "[::]:0"
                        };
                        let loc_conn = match net2.bind(SocketAddr::from_str(bind_address)?).await {
                            Ok(c) => c,
                            Err(err) => {
                                log::warn!(
